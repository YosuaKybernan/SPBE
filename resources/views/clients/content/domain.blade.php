...
@section('title', 'SPBE - ' . $domain->title)
...
@section('content-body')
    <main id="main">
        <!-- ======= Domain Section ======= -->
        <section id="domain" class="about mt-5">
            <div class="container mt-5" data-aos="fade-up">
                <div class="section-title">
                    <h2>Domain {{ $domain->title }}</h2>
                </div>
                <div class="row content">
                    <div class="col-lg-12">
                        <div class="accordion" id="accordionDomain">
                            @foreach ($domain->aspects as $index => $aspect)
                                <div class="accordion-item">
                                    <h2 class="accordion-header">
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#collapseDomainAspect{{ $index + 2 }}" aria-expanded="false"
                                            aria-controls="collapseDomainAspect{{ $index + 2 }}"
                                            style="font-size: 20px; font-weight: 500">
                                            {{ $aspect->title }}
                                        </button>
                                    </h2>
                                    <div id="collapseDomainAspect{{ $index + 2 }}" class="accordion-collapse collapse"
